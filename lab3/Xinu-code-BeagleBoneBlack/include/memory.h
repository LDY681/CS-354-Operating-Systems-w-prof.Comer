...
/*----------------------------------------------------------------------
 *  mymeminit  --  create an array of structures
 *----------------------------------------------------------------------
 */
struct blkList {	/* node of single linked list */
	char	*blockaddr;
	struct blkList *next;
};
struct memEntry {	/* entry of array arrStruct */
	uint32 blockSize;
	struct blkList *list;
};
extern struct memEntry arrStruct[20];		// array of structures.
